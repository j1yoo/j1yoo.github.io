...
| `_data/academic_profile.yml` → `service` | About summary and CV journal/conference reviewing and editorial service; organizing roles in the CV |
...
About remains a selected, authored introduction. It need not reproduce every CV award or grant. Its introductory prose is not entirely generated from structured data; when changing a fact that appears in that prose, review it too. The full web CV is `/cv_print/`.
...
1. Edit the canonical records and any deliberately authored prose affected by the change.
